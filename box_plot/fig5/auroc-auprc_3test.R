...
metric_ = 'per peak auPRC'
r = 4

filtered_df <- df %>% filter(Metric == metric_)  

##############----------auROC/auPRC--------#########
## "auROC","per cell auROC" / "per peak auROC"
## "auPRC","per cell auPRC" / "per peak auPRC"

#########----------filter human & mouse which are not paired-------##########
...
write_csv(out, paste0(metric_,'_r',r,"_test.csv"))
...
ggsave(paste0(metric_,'_r',r,'.pdf'), plot = last_plot(), 
