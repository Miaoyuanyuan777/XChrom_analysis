...
##############----------auROC--------#########
filtered_df <- df %>% filter(Metric == "per cell auPRC")
## per cell auPRC
## per peak auPRC
...
y_pos <- filtered_df %>% 
  ...
  mutate(y_position = max_val + 0.04)  # 根据实际数据调整偏移量
